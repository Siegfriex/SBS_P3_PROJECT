# P3_TARGET — Marked Audit Results 2020 / 2024

문화체육관광위원회 국정감사 처리결과 보고서의 PDF Highlight annotation을 행 선택 프록시로 사용해, 마킹된 요구사항과 대응 조치를 구조화한 병렬 준비 브랜치입니다.

이 디렉터리와 브랜치의 작업 범위는 `P3_TARGET/`으로 제한합니다. 2020년과 2024년만 처리하며 2021·2022·2023년은 현재 범위에서 제외합니다.
...
각 노트북은 annotation audit, 목차·본문 행 파싱, `issue_no` 매핑, 본문 anchor 분리, 상태 분류, 조건부 OCR, 2차 논제 검토, export 및 재로딩 검증을 독립적으로 수행합니다. 두 노트북 모두 전체 실행을 완료했으며 코드 셀 14개에 실행 번호와 결과가 저장돼 있습니다.
...
- 모든 Highlight annotation은 목차 또는 본문 `issue_no`로 연결됐습니다.
- 모든 선택 논제의 `issue_text`가 추출됐고 `issue_id`는 중복이 없습니다.
- 조건부 OCR은 2020년 4건, 2024년 1건 시도했으며 OCR로 대체된 행은 없습니다.
- 상세 품질 결과는 `reports/2020_MARKED_MAPPING_QUALITY.md`와 `reports/2024_MARKED_MAPPING_QUALITY.md`에 있습니다.
